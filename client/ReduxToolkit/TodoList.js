...
const TodoList = () => {
  ...
  console.log({ filterState, todoListState, todoList });

  useEffect(() => {
    if (todoListState === todoListStates.idle) {
      dispatch(loadTodos(2000));
    }
  }, []);
  ...
  const onChangeFilter = filterButtonState => () => dispatch(changeFilter(filterButtonState));
  const onChangeTodoStatus = id => () => dispatch(changeTodoStatus(id));
  const onAddNewTodo = (values, fm) => {
    fm.resetForm();
    dispatch(addNewTodo(values.newTodoText));
  };
